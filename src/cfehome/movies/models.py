...
class MovieQuerySet(models.QuerySet):
    ...
    def popular(self, reverse=False):
        ordering = self.annotate(score=-Sum(F('rating_avg') * F('rating_count'))).order_by('score')
        if reverse:
            ordering = self.annotate(score=Sum(F('rating_avg') * F('rating_count'))).order_by('score')
        return ordering
...
class Movie(models.Model):
    ...
    poster_path = models.CharField(max_length= 3000, unique=False)
    ratings = GenericRelation(Rating) # Query set User.ratings
